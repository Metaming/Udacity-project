...
features_list = ['poi','salary','bonus','total_payments','from_this_person_to_poi','from_poi_to_this_person','from_messages','to_messages','shared_receipt_with_poi'] # You will need to use more features
...
features_dict = {}
features_array = pd.DataFrame(index=data_dict.keys())
### number of valid features for poi
valid_feature_poi = pd.Series(np.zeros(len(features_list)),index=features_list)
...
for key, info in data_dict.items():
    ...
    for ft in features_list:
        if data_dict[key][ft] == 'NaN':
            data_dict[key][ft]=0

        if  data_dict[key][ft]!=0 and data_dict[key]["poi"]==True:
            valid_feature_poi[ft]+=1
            
        features_dict [key][ft] = data_dict[key][ft]
        
        ## Add new data to the DataFrame, using .loc
        features_array.loc[key,ft]= data_dict[key][ft]
...
#features_array
print "number of valid_feature_poi\n",valid_feature_poi


# remove the max that corresponds to the "total"
index_max = features_array.iloc[:,1].argmax()
features_array=features_array.drop(index_max)
...
features_array.head()    
#features_array['ratio_from_message'] = features_array[features_list[3]]/ features_array[features_list[5]]
#ratio_to_message = features_array[features_list[4]]/ features_array[features_list[6]]
...
features_dict.pop("TOTAL", 0 )
### Task 3: Create new feature(s)
for key in features_dict.keys():
    
    features_dict[key]['ratio_from_message']= features_array.loc[key,'ratio_from_message']
    features_dict[key]['ratio_to_message']= features_array.loc[key,'ratio_to_message']
    
#data_dict[key]['ratio_to_message']

### Store to my_dataset for easy export below.
my_dataset = features_dict
